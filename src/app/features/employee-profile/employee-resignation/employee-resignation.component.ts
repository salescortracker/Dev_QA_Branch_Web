...
deleteResignation(id: number) {
  ...
  resetForm(form: NgForm) {
   ...
   // Manager Approval
  approveByManager(item: EmployeeResignation) {
    const model = { ...item, status: 'Approved by Manager', managerApprovedDate: new Date() };
    this.resignationService.update(item.resignationId!, model).subscribe(() => this.loadResignations());
  }

  rejectByManager(item: EmployeeResignation) {
    const model = { ...item, status: 'Rejected by Manager', managerRejectedDate: new Date() };
    this.resignationService.update(item.resignationId!, model).subscribe(() => this.loadResignations());
  }

  // HR Approval
  approveByHR(item: EmployeeResignation) {
    const model = { ...item, status: 'Approved by HR', hrApprovedDate: new Date() };
    this.resignationService.update(item.resignationId!, model).subscribe(() => this.loadResignations());
  }

  rejectByHR(item: EmployeeResignation) {
    const model = { ...item, status: 'Rejected by HR', hrRejectedDate: new Date() };
    this.resignationService.update(item.resignationId!, model).subscribe(() => this.loadResignations());
  }
}
